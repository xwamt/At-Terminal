...
export const AT_TERMINAL_TOOL_CATALOG: ToolCatalogEntry[] = [
  ...
  },
  {
    ...
    description:
      'Run a non-interactive command on an AT Terminal SSH server. Every command opens a confirmation dialog in the IDE; only a read-only allowlist (ls, cat, grep, ps, df, systemctl status, journalctl and similar) can skip it, and only on servers the user marked as trusted. A pipeline skips confirmation only when every one of its stages is on that allowlist; redirects, chaining, subshells and background jobs always ask. Expect a human round-trip and do not batch work into one shell line to avoid it. stdout/stderr each default to 64000 bytes (hard cap 256000); when truncated is true, narrow the command—do not dump whole configs (for example nginx -T or docker compose config).',
    risk: 'exec',
    inputSchema: {
      type: 'object',
      properties: {
