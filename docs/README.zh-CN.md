...
MCP hub 不直接读取 AT Terminal 的密码、私钥或服务器配置。实际调用链是：

1. AT Terminal MCP 扩展在 VS Code、Kiro、Cursor 等兼容 IDE 中启动。
2. 扩展启动本地 localhost bridge，并发布到 `~/.at-series` 注册表。
3. 扩展将打包的 `dist/hub.js` 选举到 `~/.at-series/mcp/hub.js`。
4. MCP Client 通过 `node ~/.at-series/mcp/hub.js` 启动共享 **AT Series** MCP server。
5. Hub 连接回本地 bridge。
6. 扩展使用已有的 AT Terminal 配置、SecretStorage、主机指纹校验和确认弹窗执行工具调用。
