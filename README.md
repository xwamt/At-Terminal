...
- `at-terminal-0.2.11.vsix`: 基础版，不含 MCP 工具。
- `at-terminal-mcp-0.2.11.vsix`: MCP 版，包含 Agent 工具和 stdio MCP server。

安装 MCP 版：

```powershell
code --install-extension .\at-terminal-mcp-0.2.11.vsix
```
...
```json
{
  "mcpServers": {
    "AT Terminal": {
      "command": "node",
      "args": [
        "C:/Users/alan/.kiro/extensions/local.at-terminal-mcp-0.2.11/dist/mcp-server.js"
      ],
      "autoApprove": [
        "list_ssh_servers",
...
如果出现 `MODULE_NOT_FOUND`，检查 `args[0]` 是否指向 Kiro 的扩展安装目录，例如 `.kiro/extensions/local.at-terminal-mcp-0.2.11/dist/mcp-server.js`，不要误指向 VS Code 的 `.vscode/extensions`。
...
```json
{
  "mcpServers": {
    "AT Terminal": {
      "command": "node",
      "args": [
        "C:/Users/alan/.cursor/extensions/local.at-terminal-mcp-0.2.11/dist/mcp-server.js"
      ]
    }
  }
}
...
```json
{
  "mcpServers": {
    "AT Terminal": {
      "command": "node",
      "args": [
        "${userHome}/.cursor/extensions/local.at-terminal-mcp-0.2.11/dist/mcp-server.js"
      ]
    }
  }
}
...
mcpServers:
  - name: AT Terminal
    command: node
    args:
      - C:/Users/alan/.vscode/extensions/local.at-terminal-mcp-0.2.11/dist/mcp-server.js
```
...
在 VS Code 中安装 `at-terminal-mcp-0.2.11.vsix` 后，Copilot Chat Agent mode 可以发现 language model tools。
...
- `at-terminal-0.2.11.vsix`: base extension without MCP tools.
- `at-terminal-mcp-0.2.11.vsix`: MCP-enabled extension with tools and stdio MCP server.
...
After installing `at-terminal-mcp-0.2.11.vsix` in VS Code, Copilot Chat Agent mode can discover the contributed language model tools.
